...
export async function GET(request:Request) {
  try { const context=await requireCrmContext(); await requireCrmFeature(context.tenantId,"sales"); const search=new URL(request.url).searchParams;return Response.json(await listOperationalSales(context.tenantId,{page:Number(search.get("page")||1),pageSize:Number(search.get("pageSize")||25),search:search.get("search")||undefined,dateFrom:search.get("dateFrom")||undefined,dateTo:search.get("dateTo")||undefined,productId:search.get("productId")||undefined,commercialPlanId:search.get("commercialPlanId")||undefined,transactionType:search.get("transactionType")||undefined,status:search.get("status")||undefined,historicalAdvisorName:search.get("historicalAdvisorName")||undefined},context.teamUserIds,context.storeId)); } catch(error){return crmError(error)}
}
...
export async function POST(request:Request){
  try{
    ...
    const agentId=context.role==="AGENT"?context.userId:body.agentId;const agent=await validateAssignableUser(context,agentId);if(agent.role.code!=="AGENT")throw new Response("El promotor debe tener rol AGENT",{status:400});
    ...
    // Store is never client-selectable for an AGENT — it's derived from their own Employee assignment (instruction:
    // "NO seleccionable manualmente"). Other roles may pass one explicitly; empresas sin sucursales simplemente nunca la envían.
    const requestedStoreId=context.role==="AGENT"?(await prisma.employee.findFirst({where:{tenantId:context.tenantId,userId:context.userId},select:{storeId:true}}))?.storeId??null:(body.storeId||null);
